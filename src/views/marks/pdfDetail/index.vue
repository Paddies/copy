<template>
  <div class="container">
    ...
    <div class="topic">
      <el-card>
        <el-row>
          <el-col><span>文件名称: </span><span style="color: #1890ff;cursor: pointer;" @click="getJobGroupTable()">{{
            fileName }}</span></el-col>
        </el-row>
        <p></p>
        <div style="display: flex;gap: 10px 30px;">
          <!-- <span>任务状态: <span style="color: purple">{{ dictLabel }}</span></span> -->
          <span>任务状态: <span :class="`status-tag status-tag-jobStatus-${groupStatu}`">{{ dictLabel }}</span></span>
      ...
      <el-card width="100%">
        ...
        <div class="form">
          <el-form ref="topic_form" :model="form" size="mini" statu-icon :rules="rules" label-width="100px"
            label-position="left">
            <el-row>
              <el-col :span="24">
                <el-form-item prop="titleType" label="题目类型：">
                  <el-radio-group v-model="form.titleType" :disabled="disabled" size="mini">
                    <el-radio v-for="item in radios" :key="item.name" :label="item.name" style="margin: 5px 20px 0 0">{{
                      item.label }}</el-radio>
                  </el-radio-group>
                </el-form-item>
              </el-col>
            ...
            <el-row>
              <el-col :span="24" align="end">
                <!-- 标注页面按钮 -->
                <div class="submit"
                  v-if="!$route.query.checker && !disabled && groupStatus !== '0' && groupStatus !== '5'">
                  <el-button type="danger" v-hasPermi="['pdf:job:cancel']" @click="handleCancellation"
                    size="mini">作废</el-button>
                  <el-button type="primary" plain v-hasPermi="['pdf:job:completeJob']" @click="okCheckOpen = true"
                    size="mini">完成标注</el-button>
                  <el-popconfirm confirm-button-text='确认' cancel-button-text='取消' icon="el-icon-info" icon-color="red"
                    title="确认后将会提交审批，是否确认？" @confirm="submit">
                    <el-button slot="reference" type="primary" v-hasPermi="['pdf:job:submitJob']"
                      :disabled="jobType === '4'" :loading="submitLoading" size="mini"
                ...
                <!-- 质检按钮 -->
                <div v-if="$route.query.checker && groupStatus !== '4'">
                  <el-button type="warning" size="mini" v-hasPermi="['pdf:job:reback']" @click="rejectForm">驳回</el-button>
    ...
    <!-- 标注、质检完成弹框 -->
    <el-dialog :title="!$route.query.checker ? '完成标注' : '完成质检'" :visible.sync="okCheckOpen" width="600px" append-to-body>
      <el-form ref="okCheckForm" :model="okCheckForm" :rules="okcheckrules" label-width="80px">
        <el-form-item label="科目" prop="subject" v-if="checkShowInput('subject')">
        ...
        <el-form-item label="语言" prop="language" v-if="checkShowInput('language')">
        ...
        <el-form-item label="出刊日期" prop="pubYear" v-if="checkShowInput('pubYear')">
          <el-input v-model="okCheckForm.pubYear" type="text" placeholder="请填入出刊日期" size="mini"
...
export default {
  ...
  computed: {
    ...
    //   return this.radios.filter(item => item.name === this.form.titleType)
  ...
  methods: {
        ...
        if (valid) {
          ...
          cancellation({
            ...
            if (res.code === 200) {
              ...
              // 回显下一道题
              this.$router.replace({ path: '/marks/pdfDetail', query: { id: res.data.id, fileId: res.data.fileId, proId: this.$route.query.proId, } })
    ...
    checkShowInput(type) {
      return this.pdf_config.includes(type)
    },
    /**
     *@Description: 获取pdf链接
    ...
    getInitPdf(jobId) {
      ...
      getJobContent({ jobId, projectId: proId }).then((response) => {
        if (response.code === 200) {
          // 控制进入的时候加载pdf到指process定页面
          const { configs } = response.data
          console.log('2024-01-03 21:00:42 index.vue line:480', configs)

          if (configs && configs.pdf_config) {
            this.pdf_config = configs.pdf_config
          }
          ...
          if (configs && configs.mark_title_type[0]) {

            this.form.titleType = configs.mark_title_type[0]
    ...
    */
    subOkCheckForm() {
      ...
      this.$confirm(`是否继续${type === 'mark' ? '标注' : '质检'}？`, '提示', {
    ...
    checkAndMarkSave(type) {
      ...
      this.$refs["okCheckForm"].validate((valid) => {
        if (valid) {
          ...
          behaviors[type](params).then(res => {
    ...
    getNewCheckDetail(detail, _next) {
      if (_next) {
        ...
        if (detail) {
          ...
          this.$route.query.fileId = detail.fileId
          this.jobId = detail.id
