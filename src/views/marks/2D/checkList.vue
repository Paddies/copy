<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData" height="700">
      ...
      <el-table-column label="文件ID" prop="fileId" width="300" show-overflow-tooltip />
      <el-table-column label="文件名称" prop="fileName" width="300" show-overflow-tooltip />
      ...
      <el-table-column label="质检员" prop="checkNickName" />
      <el-table-column label="质检时间" prop="submitTime" :formatter="formatDate" />
      <el-table-column label="任务耗时" prop="checkTimeConsumingDesc" />
...
export default {
  ...
  methods: {
    ...
    /** 详情 */
    handleUpdate(row) {
      let { groupId, groupStatus, proId } = this.$route.query
      this.$router.push({
        path: '/marks2D/check',
        query: {
          id: row.id,
