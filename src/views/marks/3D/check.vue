        ...
        <el-row :gutter="4" class="three-main">
          <el-col :span="18">
            <div class="views">
              ...
              <div id="three">
                <!-- 弹出模块，设置属性及其它信息 -->
                <attribute-modal v-if="show" :show.sync="show" :target="target" @delete="deleteObject"></attribute-modal>
                <!-- <div id="modal_setting" class="modal_setting">
                  <p class="modal_setting_title"><span>属性设置</span> <span class="close" @click.stop="deleteObject"><i class="el-icon-delete"></i></span></p>
                  <div id="modal_setting_content"></div>
                </div> -->
              </div>
            </div>
          </el-col>
          ...
          <el-col :span="24">
            <div class="keyframes">
              <span v-for="(item, index) in pcdList" :key="index" class="keyframe"
                ...
                :style="{ background: getItemColor(item.jobStatus) }"> {{ index + 1
                }}</span>
            </div>
          </el-col>
        </el-row>
      ...
      <el-col :span="5" class="objects">
        <div class="object">
          <p class="object_title">
            <i class="el-icon-d-arrow-left"></i><span>质检方案</span>
          </p>
        ...
        <div class="submit">
          <el-button type="success" size="mini" v-hasPermi="['point:cloud:completeCheck']" @click="checkPasss">
            <i class="el-icon-circle-check"></i>
            <span>通过</span></el-button>
          <el-button size="mini" type="danger" v-hasPermi="['point:cloud:completeCheck']" @click="showDialog = true">
            <i class="el-icon-circle-close"></i>
            <span>驳回</span></el-button>
          <el-button size="mini" type="primary" v-hasPermi="['point:cloud:completeJob']" @click="completeCheck()">
    ...
    <el-dialog :visible.sync="showDialog" title="驳回理由">
      <el-form :model="reasonParams" ref="rejectReason">
        ...
        <el-form-item prop="reason" label="驳回原因">
          <el-input type="textarea" v-model="reasonParams.reason" style="width: 80%;" placeholder="请输入驳回理由" clearable />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="showDialog = false">取消</el-button>
        <el-button type="primary" @click="graphJobCheck">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rejectReason: {
        //   reasonIds: [
        //   { type: 'array', required: true, message: '请选择错误类型', trigger: 'change' }
        // ],
        reason: [
          { required: true, message: '请输入驳回理由', trigger: 'blur' }
        ]
      },
      showDialog: false, // 驳回弹框
      pageLength: '',//帧数
      groupId: "",//分组id
      job_mode: "",//发题模式
      jobStatus: "",//当前帧任务状态
  ...
  methods: {
    async getOverruledList() {
      const res = await this.getDicts("pointcloud_error_type ");
    ...
    getGroupItem() {
      ...
      return getGroupItem(params, this.pageSize).then(res => {
        if (res.code === 200) {
      ...
      if (reason === '' && reasonIds.length === 0) {
        ...
        graphJobCheck({
          ...
          if (res.code === 200) {
            ...
            this.$message.success("此帧驳回成功");
            this.resetForm('rejectReason')
          }
        });
      }
    },
