<template>
  <div class="draw-container" @click.stop="show = false">
    ...
    <div v-show="show" id="customContextMenu" style="
        position: absolute;
        left: 0;
        top: 0;
    ...
    <el-dialog :visible.sync="dialogVisible" width="50%" :close-on-click-modal="false" :close-on-press-escape="false"
      :show-close="false" title="提示：请选择对应标签类型">
      <el-form :mode="labelType">
        <el-form-item prop="titleType">
          <el-radio-group v-model="labelType.value" size="mini" style="margin-top: 20px">
            <el-radio v-for="item in labList" :key="item.dictCode" :label="item.dictLabel" :name="item.dictLabel"
              style="margin-right: 40px; margin-bottom: 20px" :style="{ color: item.remark }" :fill="item.remark"
...
export default {
  ...
  async mounted() {
    ...
    document.addEventListener("createObject", (event) => {
      console.log('createObject',event);
