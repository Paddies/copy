<template>
  ...
  ">
      ...
      <el-table :data="listData" v-loading="loading">
        ...
        <el-table-column label="项目经理名称" prop="projectManagerId" :formatter="managerNameFormat" width="120" />
        <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width" width="350">
          <template slot-scope="scope">
            ...
            <!-- <el-button size="mini" type="text" icon="el-icon-edit-outline" @click="handleUpdate(scope.row)"
              v-hasPermi="['project:detail']">编辑</el-button> -->
            <!-- && scope.row.jobMode === '1' -->
            <!-- <el-button size="mini" type="text" v-if="checkPermi(jobPermision)&&scope.row.jobMode === '1'" icon="el-icon-s-promotion"
              @click="handleMark(scope.row)">快速标注</el-button>
            <el-button size="mini" type="text" v-if="checkPermi(checkPermision)&&scope.row.jobMode === '1'" icon="el-icon-s-promotion"
              @click="handleCheck(scope.row)">快速质检</el-button> -->
  ...
  export default {
    ...
    data() {
      return {
        jobPermision: ['pdf:job:quick', 'ocr:job:jobTask', 'voice:job:quickJob',],
        checkPermision: ['pdf:check:quick', 'ocr:job:checkTask', 'voice:job:quickCheck'],
        // 遮罩层
        loadingUser: true,
        loading: true,
        // 总条数
