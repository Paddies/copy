...
export default {
  ...
  data() {
    return {
      ...
      deptNodeAll: false,
      //文件上传地址
      fileUrl: process.env.VUE_APP_BASE_API + "/tools/upFileVoice",
      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 50,
  ...
  methods: {
    ...
    beforeVoiceUpload(file) {
      const isMp3 = file.type === 'audio/mp3' || file.type === 'audio/wmr' || file.type === 'audio/mpeg' || file.type === 'audio/ogg';
      if (!isMp3) {
        this.$message.error('上传只能是 .mp3,.wmr格式！');
        this.viceList = {};
        return false;
      }
    },
